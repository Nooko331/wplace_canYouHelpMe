...
遗漏点在空间上呈现“孤岛”特征：少量同色点被一圈未匹配颜色的“护城河”包围，护城河外围又是同色大块已处理区。算法据此三条件判定：**小簇 + 护城河 + 外围同色大簇**。第三个条件用于区分“漏涂残留”与“图案本身就该独立的小色块”——前者外围一定有同色大簇（它本该属于那片已处理区），后者外围是别的颜色。

## 两种触发方式

1. **自动触发**  
   勾选“启用遗漏检测”后，每次“自动检测及填充”或“全自动检测及填充”完成后，会自动对刚处理区域检测遗漏点。

2. **手动触发**  
   点击“运行遗漏检测”按钮，对当前框选范围检测遗漏点。
